  ...
  if (timeout > 0)
    {
      ...
      regval &= ~STM32_PWR_CR3_BYPASS;       // Use internal regulator (not external supply)
      regval &= ~STM32_PWR_CR3_SDEN;         // For internal regulator, use LDO (not SMPS)
      regval |= STM32_PWR_CR3_LDOEN;         // Enable the internal LDO
